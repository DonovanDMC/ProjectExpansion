...
public class TileTransmutationInterface extends TileNBTFilterable implements ITickableTileEntity {
    ...
    class ItemHandler implements IItemHandler {
        ...
        public ItemStack getStackInSlot(int slot) {
            ...
            if (slot <= 0 || fetchKnowledge().length < slot) return ItemStack.EMPTY;
        ...
        public ItemStack extractItem(int slot, int amount, boolean simulate) {
            if (slot <= 0 || owner == null || fetchKnowledge().length < slot || Util.getPlayer(owner) == null)
                return ItemStack.EMPTY;
            fetchKnowledge();
