...
public class BlockEMCImport extends ContainerBlock {
    ...
    public void addInformation(ItemStack stack, @Nullable IBlockReader level, List<ITextComponent> list, ITooltipFlag flag) {
        ...
        list.add(new TranslationTextComponent("block.projectexpansion.emc_import.tooltip").setStyle(ColorStyle.GRAY));
    }
        ...
        if (!world.isRemote) {
            ...
            if (tile instanceof TileEMCImport) {
                ...
                if (!t.owner.equals(player.getUniqueID())) {
                    player.sendStatusMessage(new TranslationTextComponent("block.projectexpansion.emc_import.not_owner", new StringTextComponent(t.ownerName).setStyle(ColorStyle.RED)), true);
