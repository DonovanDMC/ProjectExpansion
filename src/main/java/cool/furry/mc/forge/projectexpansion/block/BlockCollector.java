...
public class BlockCollector extends Block implements IHasMatter {
    ...
    public void addInformation(ItemStack stack, @Nullable IBlockReader level, List<ITextComponent> list, ITooltipFlag flag) {
        ...
        list.add(new TranslationTextComponent("block.projectexpansion.collector.emc", new StringTextComponent("").appendSibling(TransmutationEMCFormatter.formatEMC(matter.getCollectorOutputForTicks(Config.tickDelay.get()))).setStyle(ColorStyle.GREEN)).setStyle(ColorStyle.GRAY));
    }
