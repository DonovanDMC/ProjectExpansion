...
public class CommandEMC {
    ...
    private static int handle(CommandContext<CommandSource> ctx, ActionType action) throws CommandSyntaxException {
        ...
        switch (action) {
            ...
            case SET: {
                newEMC = value;
                if (compareUUID(ctx.getSource(), player)) ctx.getSource().sendFeedback(new TranslationTextComponent("command.projectexpansion.emc.set.successSelf", formatEMC(value), formatEMC(newEMC)).setStyle(ColorStyle.GREEN), false);
                else {
