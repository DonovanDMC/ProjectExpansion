...
public class TileRelay extends TileEntity implements ITickableTileEntity, IEmcStorage {
    ...
    public TileRelay() {
        super(Objects.requireNonNull(TileEntityTypes.RELAY.get()));
    }

    @Override
    public void read(@Nonnull CompoundNBT nbt) {
        super.read(nbt);
        if (nbt.contains(NBTNames.STORED_EMC, Constants.NBT.TAG_LONG)) emc = new BigInteger(nbt.getString(NBTNames.STORED_EMC));
    }
    ...
    public void tick() {
        ...
        List<IEmcStorage> temp = new ArrayList<>();
